...
| Five self-diagnosed A3/A5/D2 tests laundered under an UNFALSIFIABLE heading (`Tests/Editor/Unit/MathfloatPTest.cs`, `floatPTests.cs`) | CLOSED | Closed 2026-08-04. The four `MathfloatP` determinism tests now assert golden `RawValue` literals harvested from the reference implementation (sanity-checked against real math: Sin(0.5)=0.4794, Sqrt(2.5)=1.5811, Exp(2.5)=12.18). Each was observed RED in the **Editor**: Bhaskara constant `0x42456460`->`0x42456461` reddens `Determinism_VerifyRawValues` (1090413845->1090413844) and the Sin row of `AllTrigFunctions`; `Cos` -> `Sin(x)` reddens `CrossPlatform` (1065965998->1075700924) and the Cos row; `Sqrt` unbias `m -= 127`->`126` reddens `AllPowerFunctions` (1070228162->1074731965). `floatPTests.ExplicitConversion_ToFloat` deleted after proving A5 — `operator floatP(float)` with `f * 2f` reddened it and `ImplicitConversion_FromFloat` with identical expected and actual (2.46 for 1.23), which is the surviving sibling. All production files restored byte-identical (md5 verified). | 2026-08-04 |
